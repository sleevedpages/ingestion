...
- **⚠️ OUTBOUND contract (session 40; `value-snapshots` + `price-anomaly-scan` + — since 2026-08-20 —
  `ebay-order-sync`).** Every other job runs
  entirely inside this worker. These three are each a single authenticated POST INTO the Content app
  (`/api/internal/snapshots/run` · `/api/internal/price-anomalies/run` · `/api/internal/ebay/sync-orders`), which reverses the direction
