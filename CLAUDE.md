...
| `0 8 * * *` | **DAILY** PRICE ANOMALY SCAN trigger → `runPriceAnomalyScan()` (`src/priceAnomalyScan.ts`; job id `price-anomaly-scan`). **The SECOND job that calls out into Content and computes nothing** (the `value-snapshots` seam, exactly): POSTs `{CONTENT_APP_URL}/api/internal/price-anomalies/run` with `x-worker-secret` and logs the returned counts. Content runs the report-only pricing sentinel (Content mig **0129**, born from the 2026-08-16 Mega Dragonite $32,000 incident): graded-ceiling / overnight-spike / absolute-ceiling detection against the ONE provenance chain (`MARKET_PRICE_CHAIN_SQL`) → `price_anomalies` rows for Admin → Price Anomalies. Re-implementing any rung here would fork the chain — never do it. Same `CONTENT_APP_URL` no-default rule as value-snapshots (absent → self-skip; a prod fallback would let a UAT worker scan/write PROD). Failure contract identical: genuine failure THROWS → honest `status='error'` run-log row; `.catch(logger.error)` = log-and-continue. **Why 08:00 UTC:** first free hour AFTER the whole price-ingest block (04:00 drain / 05:00 PC fetch+queue / 06:00 TCG sync; 07:00 is news-poll) and BEFORE the 10:00 value snapshot, so a currency-scale anomaly is flagged before it lands in the value series. Prod only. Tests: `src/priceAnomalyScan.test.ts`, `src/priceAnomalyScanCron.test.ts`. |
| `*/15 * * * *` | **EVERY 15 MIN** eBay ORDER SYNC trigger → `runEbayOrderSync()` (`src/ebayOrderSync.ts`; job id `ebay-order-sync`). **The THIRD job that calls out into Content and computes nothing** (the `value-snapshots`/`price-anomaly-scan` seam, exactly): POSTs `{CONTENT_APP_URL}/api/internal/ebay/sync-orders` with `x-worker-secret` and logs the counts. Content polls each linked eBay seller account (sell.fulfillment getOrders), records order lines under the mig-0133 idempotency key, writes `vendor_sales` (`channel='ebay'`, `source='ebay_sync'`) and DECREMENTS inventory through its ONE checkout core. **THIS WORKER HOLDS NO EBAY SELLER TOKEN** — its own `EBAY_CLIENT_ID/SECRET` are the unrelated Browse-UPC app credentials; never conflate. The 15-min tick is the FLOOR: Content's `app_config.ebay_order_sync_interval_min` (default 15) is the effective per-account cadence, and Content self-gates on `ebay_listings_enabled` + `ebay_order_sync_enabled`, so a dark/disabled feature makes every tick a cheap no-op (`skipped` in the run log, still `ok`). Same `CONTENT_APP_URL` no-default rule (absent → self-skip; a prod fallback would let a UAT worker decrement PROD inventory). Failure contract identical: genuine failure THROWS → honest `status='error'` run-log row; `.catch(logger.error)` = log-and-continue; the next tick is the retry (everything Content-side is idempotent — a re-fire can never double-decrement). Prod only. On-demand = the EbayCard "Sync now" button (a Content vendor route), not an admin job here. Tests: `src/ebayOrderSync.test.ts`, `src/ebayOrderSyncCron.test.ts`. |
| `0 12 * * *` | **DAILY** PRICE ARCHIVE CAPTURE trigger → `runPriceArchive()` (`src/priceArchive.ts`; job id `price-archive-capture`). **The FOURTH job that calls out into Content and computes nothing — with a LOOP.** It POSTs `{CONTENT_APP_URL}/api/internal/price-archive/run` with `x-worker-secret` repeatedly (cap 30 calls) until the day reports `done`, because a full capture (~1.33M `prices` rows, measured 2026-08-31) cannot fit one Content invocation; Content's endpoint does ONE bounded batch per POST and RESUMES from R2 part metadata. Content owns everything — the prices table, the day convention (`snapshot_tz_offset_min`, the same fixed offset as the value snapshots) and the private `PRICE_ARCHIVE` bucket; re-implementing any of it here would fork the data model. Same `CONTENT_APP_URL` no-default rule (absent → self-skip; a prod fallback would let a UAT worker drive PROD captures). Failure contract identical: genuine failure THROWS → honest `status='error'` run-log row; `.catch(logger.error)` = log-and-continue; landed parts are KEPT and a manual re-fire the same day resumes. **Why 12:00 UTC:** the capture must land AFTER the day's last bulk price write, and the 04/05/06 block's QUEUES drain for hours past their trigger times — 12:00 gives the 06:00 TCG fan-out six hours, sits clear of the 10:00 snapshot/watch pair and precedes the 16:00 watch lane. Prod only. Tests: `src/priceArchive.test.ts`, `src/priceArchiveCron.test.ts`. |
| `0 13 * * *` | **DAILY** PRICE DAILY PROJECTION trigger → `runPriceDailyCapture()` (`src/priceDailyCapture.ts`; job id `price-daily-capture`). **The FIFTH job that calls out into Content and computes nothing — the price-archive seam, looped.** It POSTs `{CONTENT_APP_URL}/api/internal/price-daily/run` with `x-worker-secret` repeatedly (cap 120 calls) until Content reports `done`: Content reads its OWN completed R2 daily captures (only days with a manifest, oldest first — catch-up, never backfill) and projects the TCGplayer-basis rows into `price_daily` in the dedicated price DB (`capture_method='daily_capture'`, write-on-change, retention = Content `app_config`). This worker projects nothing. **Why 13:00 UTC:** it reads what the 12:00 capture writes; every prod capture to date completed by ~12:04, so 13:00 is ~15× headroom, and a capture still in flight has no manifest yet → that day is skipped this firing and projected tomorrow. Failure contract identical to the archive trigger (throw → honest error row; `.catch(logger.error)`; progress persists in Content's `price_daily_days`, the next fire resumes). Prod only. Tests: `src/priceDailyCapture.test.ts`, `src/priceDailyCaptureCron.test.ts`. |
...
| `GET` | `/pricecharting/upc?upc=` | `x-worker-secret` | **LIVE UPC → canonical lookup** (Sealed UPC lookup Phase 1, 2026-08-13; Content mig 0127). ⚠️ **RETIRED FROM THE LOOKUP LADDER 2026-08-14** (Content mig 0128 replaced it with the `/ebay/upc` + `/upcitemdb/upc` rungs below — PriceCharting's UPC data proved unusable and this route's 1 req/sec sleeps are scanner-hostile). The route survives for ops/manual use and still stamps the map on a validated hit. Needs **`PRICECHARTING_TOKEN`** → 503. Normalizes to digits (8–14 → else 400), queries live `/api/product?t=&upc=` for BOTH barcode forms (EAN-13 leading-0 ↔ UPC-A; ≤2 calls, 1 req/sec sleep between — the graded-lookup pattern), then validates before trusting: stamped map row by pc_id (reuse, never re-match) → `validateTcgIdMatch` on a numeric `tcg-id` → else `{found:false}` (**never a weak fuzzy match**). A validated hit STAMPS `pricecharting_products` (upc + mapping, stored-stamp-wins COALESCE; skipped when the game is outside the 4 categories — game_category NOT NULL) so the next scan is a D1 hit. KV `pc_upc:{code}` — 24h positive / 6h definitive-negative, transient failures never cached. Token never logged/returned. `src/lib/pricechartingUpc.ts`; tests `src/lib/pricechartingUpc.test.ts`. |
| `GET` | `/ebay/upc?upc=` | `x-worker-secret` | **eBay Browse UPC resolver** (UPC resolution ladder, 2026-08-14; Content mig 0128) — the PRIMARY external rung behind Content's `GET /api/products/upc-lookup` (consulted after the `product_upcs` map misses; Content writes the hit back to the map, so this is spent at most once per code). One Browse GTIN search (`item_summary/search?gtin=&limit=20`, header `X-EBAY-C-MARKETPLACE-ID: EBAY_US`) → majority-signal listing-title aggregation (`src/lib/upcTitleMatch.ts` — norm + listing-noise strip, tokens in ≥ half the titles survive) → the SHARED catalogue matcher (`pickNumberlessCanonicalMatch`: all-name-tokens, language gate, set corroboration, UNIQUE accept — every gate fails toward rejection; never a second matcher). **Token mgmt:** client-credentials OAuth (`EBAY_CLIENT_ID`/`EBAY_CLIENT_SECRET`, worker-side ONLY), app token KV-cached (`ebay_oauth_token`) until ~5 min before its ~2h expiry — NEVER fetched per lookup; one forced refresh on a Browse 401. **Quota:** KV result cache `ebay_upc:{code}` (30d positive / 24h definitive-negative; transient failures never cached) + day counter `ebay_upc_calls:{date}` stopping at the `upc_ebay_daily_cap` app_config soft cap (code default 4000, under the ~5k/day quota). Missing credentials → clean miss `{found:false, skipped:'not_configured'}`, NEVER a 500. ⚠️ **eBay account-deletion notifications: OPTED OUT** (operator decision 2026-08-14 — we retain NO eBay user data, the exemption's condition; if a future feature stores eBay user data, revoke the exemption and build the challenge endpoint then). `src/lib/ebayUpc.ts`; tests `src/lib/ebayUpc.test.ts` + `src/lib/upcTitleMatch.test.ts`. **2026-08-21 — this rung had NEVER RUN in prod** (no credentials on the deployed version; see "A worker secret belongs to a VERSION" below), and the shared matcher was zeroing its own candidate pool. Fixed: provider titles are ASCII-FOLDED before tokenising (`norm()` was shattering "Pokémon" into the junk tokens `pok`+`mon`, either of which could win a discriminator slot and guarantee an empty pool); an EMPTY pool drops the weakest discriminator and retries ONCE (floor 2 — a retail tail like "… & Box Topper" or an era prefix like "Scarlet & Violet" was zeroing it before the matcher ever ran), but NEVER relaxes after the matcher REJECTS a non-empty pool (that is a verdict, not a pool defect); and every resolution emits one `upc_rung_resolution` info line with the outcome and, on a near-miss, the `reject_stage`. **"Confident" is now DEFINED** in the `upcTitleMatch.ts` header — four gates, all failing toward rejection, and the sibling-nesting AMBIGUITY REFUSAL (`Mega Evolution Booster Box` ⊂ `… Enhanced Booster Box`) is deliberate: never add a most-specific-wins tie-break, a wrong hit writes a PERMANENT map row. |
| `GET` | `/upcitemdb/upc?upc=` | `x-worker-secret` | **UPCitemdb UPC resolver** (2026-08-14) — the SECONDARY external rung (after the map AND eBay miss). KEYLESS `/prod/trial/lookup` by default (no key at that tier, HARD 100/day + ~6/min burst); `UPCITEMDB_KEY` present → `/prod/v1/lookup` with the `user_key`/`key_type` headers (both supported, keyless default). Retail titles → the SAME shared aggregation + matcher as the eBay rung. **Quota discipline:** `upc_upcitemdb_daily_cap` app_config soft cap (code default 60 — deliberately well under 100), KV cache `upcitemdb_upc:{code}` (30d/24h), and a **429 arms a 10-min KV backoff** (`upcitemdb_backoff`) — every lookup inside it is an instant miss, never a retry loop. `src/lib/upcitemdbUpc.ts`; tests `src/lib/upcitemdbUpc.test.ts`. |
| `POST` | `/upc/cache/invalidate` | `x-worker-secret` | **UPC RESOLVER CACHE BUST** (2026-08-21). Body `{ upc }` (8–14 digits, non-digits stripped). Deletes BOTH external rungs' KV entries (`ebay_upc:` + `upcitemdb_upc:`) in BOTH barcode forms (12-digit UPC-A ↔ 0-prefixed EAN-13) → `{ ok, upc, keys, deleted }`. **Why it exists:** the rungs negative-cache a definitive miss for 24h and positive-cache a hit for 30d, so a matcher fix could not be verified against any code already tested, and a newly-listed product stayed dead for a day. Idempotent + non-destructive — the durable record is Content's `product_upcs` map, never KV; worst case the next scan costs one provider call. **The key format lives HERE, not in Content** (`src/lib/upcCache.ts`) — Content could write the shared KV directly but that mirror would rot on a rename; it calls this route over the seam it already uses for `/ebay/upc`. Called automatically by Content's `PUT`/`DELETE /api/admin/upc-mappings`; operator-callable. Tests: `src/lib/upcCache.test.ts`, `src/upcCacheRoute.test.ts`. |
...
| `price-anomaly-scan` | `runPriceAnomalyScan(env)` — POST Content's `/api/internal/price-anomalies/run`; Content runs the report-only pricing sentinel (Content mig 0129) and records `price_anomalies` rows for Admin → Price Anomalies | `0 8 * * *` (PROD only) | ✅ safe to re-fire — detection upserts per (product, rule), baseline refresh change-filtered. No API key. Needs `CONTENT_APP_URL` → **503** when unset. **Use this to run the sentinel on demand** (e.g. right after a suspect ingest) or on UAT (no cron there). |
| `price-archive-capture` | `runPriceArchive(env)` — LOOP Content's `/api/internal/price-archive/run` (bounded, resumable batches) until the day reports `done`; Content archives the whole `prices` table into the private `PRICE_ARCHIVE` R2 bucket, one partition per local day (Price Index Capture Phase 1) | `0 12 * * *` (PROD only) | ✅ idempotent per local day — a completed day short-circuits on its manifest; a partial day RESUMES from the last landed part; nothing is ever rewritten. ⚠️ **MEASURED ON THE FIRST PROD DAY (2026-09-01): the MANUAL trigger's fetch-event `waitUntil` is cancelled after ~2 min (~29 of ~67 parts)** — a full manual day takes ~3 fires, AND each cancelled fire strands the KV job lock until its 30-min TTL (the `finally` release never runs) so re-fires 409 in between; the fastest manual finish is looping the Content endpoint directly with `x-worker-secret` (exactly what `runPriceArchive` does). The CRON's scheduled-event budget (15 min) fits a whole day in one firing. A cancelled fire also leaves an honest `finished_at NULL` run-log row → the panel shows `stuck (died mid-stage)`; expected, self-clears on the next run. `done:false` in the run log = cut short — re-fire the SAME day (an unfinished day has no manifest and reads as absent; it is NEVER backfilled later). No API key, no Scrydex credits. Needs `CONTENT_APP_URL` → **503** when unset. **Use this to capture on demand or on UAT** (no cron there; UAT needs its own `-uat` bucket bound). |
| `price-daily-capture` | `runPriceDailyCapture(env)` — LOOP Content's `/api/internal/price-daily/run` (bounded, resumable batches) until it reports `done`; Content projects its OWN completed R2 daily captures into `price_daily` in the dedicated price DB (Price Index Capture Phase 2, `capture_method='daily_capture'`, TCGplayer-basis only, write-on-change) | `0 13 * * *` (PROD only) | ✅ idempotent — a completed day short-circuits on its `price_daily_days` row, a cut-short day RESUMES from its next unread part, `INSERT OR IGNORE` on the PK means a re-run writes nothing new. CATCH-UP, NOT BACKFILL: only days with an archive manifest are ever projected, oldest first. The same ~2-min manual-trigger `waitUntil` cap as the archive applies (the cron's 15-min budget fits ~4–5 days of catch-up per firing; the initial ~14-day catch-up therefore completes over the first few crons, or by looping the Content endpoint directly with `x-worker-secret`). `done:false` in the run log = cut short — the next fire resumes. No API key, no Scrydex credits. Needs `CONTENT_APP_URL` → **503** when unset. **Use this to project on demand or on UAT** (no cron there). |
| `child-consent-reap` | `runChildConsentReap(env)` (`src/childConsentReap.ts`) — ONE authenticated POST to Content's `/api/internal/child-consents/reap`: Content deletes (up to 25 per run) the child accounts whose parental consent was never CONFIRMED within 7 days, each through its ONE account-deletion plan (Content child-accounts S2, 2026-09-22). This worker deletes nothing and logs counts only — never an id | `0 9 * * *` (PROD only) — the SAME slot as the trade-talk photo reap, as a SEPARATE `waitUntil` promise (either failing never stops the other) | ✅ idempotent — re-fire while `remaining > 0`. HOUSEKEEPING, not the control: Content refuses an expired consent at READ time whether or not this ran. Needs `CONTENT_APP_URL` → **503** when unset (never a fallback origin — a UAT worker must never delete PROD children). Cross-repo order: Content's endpoint deploys FIRST. **Use this to reap on demand or on UAT** (no cron there). |
...
- **⚠️ OUTBOUND contract (session 40; `value-snapshots` + `price-anomaly-scan` + — since 2026-08-20 —
  `ebay-order-sync` + — since 2026-08-31 — `price-archive-capture` and, per its own row above,
  `trade-talk-image-reap` + — since 2026-09-22 — `child-consent-reap`).** Every other job runs
  entirely inside this worker. These are each authenticated POSTs INTO the Content app
  (`/api/internal/snapshots/run` · `/api/internal/price-anomalies/run` · `/api/internal/ebay/sync-orders` · `/api/internal/trade-talk-images/reap` · `/api/internal/price-archive/run` — the archive one LOOPED until done), which reverses the direction
...
⚠️ **The `tcg-sync` run-log row measures the ORCHESTRATOR, not the catalogue write.**
`runStage('tcg-sync','sync', runIngestion)` wraps only steps 1–2 plus the enqueue, so every prod
run finishes in **~10 seconds** with `status='success'` while the consumers are still draining —
and the consumers write **no `ingestion_run_log` row at all**. A green `sync` row therefore means
"fan-out dispatched", never "catalogue written". Track a real sync through
`tcg_sync_log.groups_completed` vs `groups_enqueued`, or through the row counts of whatever the
consumers write. Logged as an observability follow-up in Content's `open-backlog.md`.
...
- ⚠️ **THE `@` NAMESPACE — the table is MULTI-SOURCE as of 2026-08-12 (Session 3A).** TCGCSV is no
  longer the only writer: sources that fill gaps TCGCSV provably cannot (first: Magic mana cost +
  colour) write keys as **`@<source>.<field>`** (`externalAttributeKey`, source id `[a-z0-9]+`).
  The rules, all in `productAttributes.ts`:
  - the TCGCSV delete is scoped **`AND name NOT LIKE '@%'`** — it still owns every un-namespaced
    key, including a NEW TCGplayer key, so nothing of its own can strand;
  - an external writer deletes only `@<its source>.%` (`externalAttributeStatements`) and **NEVER
    stamps `products.extended_data_hash`** — that column vouches for the TCGCSV rows alone;
  - `extractProductAttributes` DROPS a raw `extendedData` key starting with `@`, so upstream can
    never forge another source's provenance;
  - field names stay VERBATIM for every source; a list value is `;`-joined like every other
    multi-value in the table. Never widen the TCGCSV delete back to a bare `WHERE product_id = ?`.
  Decision + rejected alternatives (a `source` column — the PK `(product_id, name)` collision it
  does NOT solve; a sibling table — a heavier Content read):
...
- Tests: `src/lib/productAttributes.test.ts` (per-game fixtures incl. malformed/missing, the `@`
  namespace + the external builders), `src/ingestion/db.test.ts` (the change guard, batching,
...
⚠️ **ACCOUNT REFUSALS — 402 AND 403, not just 403 (fixed 2026-08-12).** `isScrydexRefusal(status)` /
`SCRYDEX_REFUSAL_STATUSES` is the ONE predicate: **403** = CREDIT_CAP_HIT, **402** = the plan is not
serving this key. Every per-expansion/per-card loop circuit-breaks on it (`scrydexProcessor`,
`scrydexImageSync`, `seedVariantProducts`/`backfillVariantImages`, `syncScrydexSetMappings`);
`scrydexEnrich` returns `skipped:'credit_cap'` for BOTH — deliberately the same string, because
Content's bulk-enrich loop stops only on `credit_guard`/`credit_cap` and a new label would leave it
looping. *Why:* from 2026-08-04 Scrydex answered 402 to everything and only 403 broke the circuit,
so the daily drain re-attempted 600–1,000 rows a day into the wall for eight days.

⚠️ **A FAILED CALL COSTS 0 CREDITS (changed 2026-08-12).** `scrydex_api_log.credits_used` is now 1
(5 for Vision) ONLY on a served response; a non-OK response or a network failure logs its real
status with `credits_used = 0`. The guard sums that column, so charging failures against it let the
402 outage book **5,295 phantom credits** in one month — on a correctly-set limit that would have
locked the worker out long after the provider recovered. Attempt volume is still visible in the log
(the row is written either way); only the billable total changed. Anything reading burn history
should compare `COUNT(*)` for attempts and `SUM(credits_used)` for spend.
...
> ✅ **`/expansions` now paginates too (fixed 2026-08-12).** It sent the ignored `limit:'500'` for as
> long as this note warned about it, so only the FIRST page of each game's expansions was ever
> mapped. Measured damage on prod: **70 of 454 Magic sets** carried a `scrydex_expansion_id`,
> capping every expansion-scoped Magic job (prices, images, the attribute fill) at ~21.7% of the
> game's products. `fetchAllExpansions()` now uses `pageSize`/`page` and stops on `totalCount`; a
> partial read is REPORTED (`complete:false`), never mistaken for the whole catalogue. Cost rose
> from 1 credit/game to 1 per page. **Re-run `/scrydex/sync-sets` once the account is live and
> re-measure the mapped-set counts before trusting any Magic coverage figure.**
...
- **Verified column list (B3-pre checkpoint, 2026-07-15 — IDENTICAL across all 4 categories; NO
  ...
  condition-18) + retail-loose-buy/sell + **upc (since 2026-08-13, Content mig 0127)**. NOT
  consumed: gamestop-*, retail-cib-*, retail-new-*, asin, epid, release-date.
- **UPC capture (mig 0127, Sealed UPC lookup Phase 1):** every row's `upc` is normalized
  digits-only (`normalizeUpc`) and written onto its `pricecharting_products` row — cards AND
  sealed (cards occasionally carry one; costs nothing). The MAP upsert's
  `upc = COALESCE(excluded.upc, stored)` means a CSV that stops publishing a UPC never blanks a
  stored one, and because EVERY row gets the map upsert each ingest (pre-stamped rows skip only
  the MATCHER), already-mapped rows gain UPCs on ordinary re-ingest — the daily rotation IS the
  backfill, no separate job. ⚠️ **AUDIT FINDING (2026-08-13, live cached files): the column was
  ALREADY in our download (no URL change — `download-custom` has no field selection) but is
  SPARSELY populated** — pokemon 79/91,668 rows (recent SEALED products only, e.g. "Ascended
  Heroes" ETBs), yugioh 0/77,426; PC also reuses one UPC across sibling products (three
  "Mega … ex Box" sharing 196214141469). So the map is a slow-growing cache and the live
  `GET /pricecharting/upc` fallback (endpoint table) is the lookup's workhorse. **Mig 0127 is
  BLOCKING for this worker deploy** (the map upsert binds `upc` unconditionally — every PROCESS
  window fails without the column).
...
| `EBAY_CLIENT_ID` / `EBAY_CLIENT_SECRET` | — | eBay client-credentials OAuth pair for the Browse UPC resolver `GET /ebay/upc` (2026-08-14). Set via `wrangler secret put` (preview + prod). **WORKER-SIDE ONLY** — never in the Content app, logs, or responses. Absent → the rung is a CLEAN MISS (`skipped:'not_configured'`), never a 500 — the lookup ladder just falls through. NO `EBAY_VERIFICATION_TOKEN` exists: account-deletion notifications are OPTED OUT (see the `/ebay/upc` endpoint row). ⚠️ **These two were NEVER PRESENT on the deployed worker until 2026-08-21** — the rung had been dark since it shipped. See "A worker secret belongs to a VERSION" below before diagnosing a repeat. |
| `UPCITEMDB_KEY` | — (**deliberately unset**) | OPTIONAL UPCitemdb paid key. Absent (the default) → `GET /upcitemdb/upc` uses the KEYLESS `/prod/trial` path; present → `/prod/v1` with the `user_key`/`key_type` headers. Only set it if the account is upgraded. |
...
## ⚠️ A worker secret belongs to a VERSION — `wrangler secret list` is NOT the deployed state (2026-08-21)

**This cost a week of a dead production feature and is the single most useful trap in this file.**
Workers secrets are part of a VERSION's binding set. `wrangler secret list` reports the **LATEST
version's** secrets; traffic is served by the **DEPLOYED** version, which is not necessarily the
latest. So "`secret list` shows it" and "the code says it is missing" can both be true, and
neither is a bug.

Measured on `sleevedpages-ingestion`: `secret list` showed `EBAY_CLIENT_ID`/`EBAY_CLIENT_SECRET`
while `wrangler versions view <deployed id>` showed the serving version carrying neither — and the
versions deployed on 2026-08-14 and 2026-08-17 (the dates the handoff recorded the eBay UPC rung
as LIVE) carried neither either. `GET /ebay/upc` had answered `skipped:'not_configured'` on every
request since the ladder shipped, and `product_upcs` held zero `ebay` rows to show for it.

- **Diagnose from the deployment, never the secret list:**
  ```bash
  npx wrangler deployments list        # which version serves traffic
  npx wrangler versions view <that id> # what bindings it ACTUALLY has
  ```
- **A dashboard-added secret is not live until a deploy.** `npm run deploy` uploads a new version
  with the current secret set and deploys it; `wrangler versions deploy <id>` promotes an existing
  one. `wrangler secret put` normally deploys as it goes — dashboard edits and `versions upload`
  do not. ⚠️ **This worker is NOT in the GitHub pipeline** (Content is); its deploy is always a
  deliberate manual step, which is a large part of why the gap persisted unnoticed.
- **Never "fix" a `not_configured` by loosening the guard.** The guard was correct — the deploy was
  missing. Every credential gate on this path now logs at `warn` naming which value was
  absent-vs-blank (`src/lib/ebayUpc.ts` → `missingEbayCredentials`; never a value or a prefix).
