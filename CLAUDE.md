...
- **⚠️ OUTBOUND contract (session 40; `value-snapshots` + `price-anomaly-scan`).** Every other job runs
  entirely inside this worker. These two are each a single authenticated POST INTO the Content app
  (`/api/internal/snapshots/run` · `/api/internal/price-anomalies/run`), which reverses the direction
