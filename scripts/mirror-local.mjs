 ...
 * mirror-local.mjs
 *
 * Mirrors card images to R2 by fetching from TCGPlayer/Skrydex using THIS
 ...
 *
 * Skrydex is not IP-blocked, so this script also handles Pokémon cards with
 * a Skrydex mapping — giving you one command to finish the full mirror.
 *
 * Usage:
 *   node scripts/mirror-local.mjs
 *   node scripts/mirror-local.mjs --skrydex-only   # only Pokémon cards with a Skrydex mapping
...
const SKRYDEX_ONLY = args.includes('--skrydex-only')

// ─── Skrydex URL construction (mirrors image-mirror.ts logic) ────────────────

function formatSkrydexCardNumber(num) {
...
function buildSkrydexUrl(setId, cardNumber) {
  return `https://images.scrydex.com/pokemon/${setId}-${formatSkrydexCardNumber(cardNumber)}/large`
}
...
async function mirrorCard(card) {
  ...
  // Pokémon with Skrydex mapping: try Skrydex first (high-res)
  if (poke && card.card_number && card.skrydex_set_id) {
    const fetched = await fetchImage(buildSkrydexUrl(card.skrydex_set_id, card.card_number))
    if (fetched) {
      await uploadImage(card.tcgplayer_product_id, fetched.buffer, fetched.contentType, 'skrydex')
      return 'skrydex'
    }
  }
...
async function main() {
  ...
  console.log(`  Batch: ${BATCH_SIZE}  Concurrency: ${CONCURRENCY}  Mode: ${SKRYDEX_ONLY ? 'Skrydex only' : 'all sources'}`)
  console.log('  Fetches images from this machine\'s IP — bypasses CDN datacenter blocks.\n')

  let batchNum = 0
  let totalProcessed = 0, totalMirrored = 0
  let totalSkrydex = 0, totalTcg = 0, totalFailed = 0
  ...
  while (true) {
    ...
    let cards
    try {
      const pendingUrl = `${WORKER_URL}/mirror/pending?limit=${BATCH_SIZE}${SKRYDEX_ONLY ? '&skrydex_only=1' : ''}`
    ...
    let batchMirrored = 0, batchSkrydex = 0, batchTcg = 0, batchFailed = 0
    for (let i = 0; i < cards.length; i += CONCURRENCY) {
      const chunk   = cards.slice(i, i + CONCURRENCY)
      const results = await Promise.all(chunk.map(mirrorCard))
      for (const r of results) {
        totalProcessed++
        if      (r === 'skrydex')   { batchMirrored++; batchSkrydex++; totalMirrored++; totalSkrydex++ }
    ...
    process.stdout.write(
      ...
      `skrydex ${pad(batchSkrydex, 2)} / tcg ${pad(batchTcg, 2)}` +
